...
class Block(nn.Module):
    def __init__(self, units, thetasDim, shareThetas=False, harmonicsNum=None):
        ...
        if isinstance(self, TrendBlock):
            assert thetasDim <= 4, 'thetasDim for TrendBlock <=4'
        assert harmonicsNum and isinstance(self, SeasonalityBlock),'only SeasonalityBlock can have harmonicsNum'
        self.units = units
...
class SeasonalityBlock(Block):
    ...
    def seasonalityModel(self, thetas, linspace):#kkk move to block
        p = thetas.size()[-1]
...
class TrendBlock(Block):
    ...
    def trendModel(self, thetas, linspace):#kkk move to block
        p = thetas.size()[-1]
...
class GenericBlock(Block):
    def __init__(self, units, thetasDim, shareThetas=False):
        if shareThetas:
            print('for TrendBlock its recommended to make shareThetas=False')
