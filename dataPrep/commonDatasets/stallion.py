...
dataInfo={'mainGroups':mainGroups, 'target':target, 'categoricalVariableGroups':categoricalVariableGroups,
               'categoricalSingularVariables':categoricalSingularVariables, 'staticCategoricals':staticCategoricals,
               'staticReals':staticReals, 'timeVaryingknownCategoricals':timeVaryingknownCategoricals, 'timeVaryingknownReals':timeVaryingknownReals,
               'timeVaryingUnknownCategoricals':timeVaryingUnknownCategoricals, 'timeVaryingUnknownReals':timeVaryingUnknownReals}
def getStallionTftProcessed(maxEncoderLength=24, maxPredictionLength=6, minEncoderLength=12, minPredictionLength=1, **dataInfo):
    ...
    df["avgVolumeByAgency"] = df.groupby(["timeIdx", "agency"], observed=True).volume.transform("mean")
    ...
    for col in ['sku', 'month', 'agency']:
        embeddingSizes[col]=[len(categoricalClasses[col]), getEmbeddingSize(len(categoricalClasses[col]))]
    classesLen=0
    ...
    for col, maxLength in zip(['encoderLength', 'decoderLength', 'sequenceLength'],
                                     [maxEncoderLength, maxPredictionLength, maxEncoderLength+maxPredictionLength]):
    ...
    notFullLenButMoreThanMinEncoderNPredictLenConditions=(df['fullLenConditions']==False) & (df['encoderLength']>=minEncoderLength) & (df['decoderLength']>=minPredictionLength)
    df.loc[notFullLenButMoreThanMinEncoderNPredictLenConditions,'notFullLenButMoreThanMinEncoderNPredictLenConditions']=True
    df.loc[~notFullLenButMoreThanMinEncoderNPredictLenConditions,'notFullLenButMoreThanMinEncoderNPredictLenConditions']=False
    ...
    datasetKwargs={'target':target, 'categoricalVariableGroups':categoricalVariableGroups,
                   ...
                   'staticReals':staticReals, 'timeVaryingknownCategoricals':timeVaryingknownCategoricals, 'timeVaryingknownReals':timeVaryingknownReals,
                   'timeVaryingUnknownCategoricals':timeVaryingUnknownCategoricals, 'timeVaryingUnknownReals':timeVaryingUnknownReals,
                   'minPredictionLength':minPredictionLength, 'maxPredictionLength':maxPredictionLength, 'maxEncoderLength':maxEncoderLength,
                   'minEncoderLength':minEncoderLength, 'timeVaryingCategoricalsEncoder':timeVaryingCategoricalsEncoder,
...
def getStallionTftDataloaders(maxEncoderLength=24, maxPredictionLength=6, minEncoderLength=12, minPredictionLength=1, mainGroups=['agency', 'sku'], batchSize=64, dataInfo=dataInfo):
    ...
    stallionTftTrainDataset=StallionTftDataset(trainDf, backcastLen=maxEncoderLength, forecastLen=maxPredictionLength,
                                               mainGroups=mainGroups, indexes=None, **datasetKwargs)
    stallionTftValDataset=StallionTftDataset(valDf, backcastLen=maxEncoderLength, forecastLen=maxPredictionLength,
                                             mainGroups=mainGroups, indexes=None, **datasetKwargs)
    stallionTftTestDataset=StallionTftDataset(testDf, backcastLen=maxEncoderLength, forecastLen=maxPredictionLength,
                                              mainGroups=mainGroups, indexes=None, **datasetKwargs)
