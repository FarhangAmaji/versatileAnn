...
def getEpfFrBeProcessed_normalizerFit_split(mainDf, backcastLen, forecastLen, trainRatio=.7, valRatio=.2):
    ...
    trainDf, valDf, testDf=splitTsTrainValTest_DfNNpDict(mainDf, trainRatio=trainRatio, valRatio=valRatio, seqLen=backcastLen+forecastLen, shuffle=False)
...
def getEpfFrBeProcessed(backcastLen=110, forecastLen=22, trainRatio=.7, valRatio=.2,
    ...
    mainDf, staticDf=getEpfFrBeProcessed_loadData(devTestMode=devTestMode, backcastLen=backcastLen, forecastLen=forecastLen)
    if rightPadTrain:
        ...
        testDf, valDf, trainDf, normalizer=getEpfFrBeProcessed_normalizerFit_split(mainDf=mainDf,
                                               ...
                                               trainRatio=1-(trainRatio+valRatio), valRatio=valRatio)
    else:
...
class EpfFrBeDataset(VAnnTsDataset):
    def __getitem__(self, idx):
        inputs={}
        inputs['target']=self.getBackForeCastData(idx, mode=self.modes.backcast, colsOrIndexes=self.targets)
        inputs['mask']=self.getBackForeCastData(idx, mode=self.modes.backcast, colsOrIndexes=['mask'])
        inputs['historyExogenous']=self.getBackForeCastData(idx, mode=self.modes.backcast, colsOrIndexes=self.historyExogenousCols)
        inputs['staticExogenous']=self.getBackForeCastData(idx, mode=self.modes.singlePoint, colsOrIndexes=self.staticExogenousCols)
        inputs['futureExogenous']=self.getBackForeCastData(idx, mode=self.modes.fullcast, colsOrIndexes=self.futureExogenousCols)
...
def getEpfFrBeDataloaders(backcastLen=110, forecastLen=22, batchSize=64,
                          ...
                          rightPadTrain=True, aggColName='price', devTestMode=False):
    trainDf, valDf, testDf, normalizer=getEpfFrBeProcessed(backcastLen=backcastLen, forecastLen=forecastLen,
                                                           trainRatio=trainRatio, valRatio=valRatio,
                                                           rightPadTrain=rightPadTrain, aggColName=aggColName, devTestMode=devTestMode)

    epfFrBeTrainDataset=EpfFrBeDataset(trainDf, backcastLen=backcastLen, forecastLen=forecastLen, mainGroups=[aggColName+'Type'], indexes=None, **datasetKwargs)
    epfFrBeValDataset=EpfFrBeDataset(valDf, backcastLen=backcastLen, forecastLen=forecastLen, mainGroups=[aggColName+'Type'], indexes=None, **datasetKwargs)
    epfFrBeTestDataset=EpfFrBeDataset(testDf, backcastLen=backcastLen, forecastLen=forecastLen, mainGroups=[aggColName+'Type'], indexes=None, **datasetKwargs)
