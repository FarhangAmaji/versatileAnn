...
from utils.vAnnGeneralUtils import DotDict, isListTupleOrSet, \
    Tensor_floatDtypeChange, isIterable
...
"# cccDevStruct eventhough npArray, tuple, df, series may contain str, which cant converted to tensor, but still we keep it this way"
knownTypesToBeTensored = DotDict({
    ...
    # cant directly be changed to tensor
    ...
    def __init__(self, obj):
        ...
        if self.type in knownTypesToBeTensored.tensor.values():
            ...
            if isTensorable(obj):
                ...
                # goodToHave2 if had taken prudencyFactor, this could have been notTensorables or stackListOfDirectTensorablesToTensor
            else:
                self.toTensorFunc = 'notTensorables'
...
def updateNestedDictPath(inputDictStyle, path, value, extendIfPossible=False):
    assert isinstance(inputDictStyle, (dict, DotDict,
                                       BatchStructTemplate)), 'inputDictStyle must be in one of dict, DotDict, BatchStructTemplate types'
    current = inputDictStyle
    if isinstance(current, BatchStructTemplate):
        current = current.dictStruct
    for i, key in enumerate(path[:-1]):
        assert isinstance(current, (
        dict, DotDict)), f'{path[:i + 1]} is not a dict or DotDict'
...
class BatchStructTemplate(TensorStacker):
    ...
    # goodToHave3 move to collateUtils
    # goodToHave3 rename to collateStruct
    ...
    def fillWithData(self, itemToAdd, path=[]):
        if not isinstance(self.dictStruct,
                          dict) and path == []:  # this is for the case we have made BatchStructTemplate of non dictionary object
        ...
        for key, value in itemToAdd.items():
            ...
            if isinstance(value, dict):
                ...
                # bugPotentialCheck1 this is making problems for shape or for set,...
    ...
    def assertIsBatchStructTemplateOrListOfBatchStructTemplates(obj):
        ...
        listOfBatchStructTemplatesCond = (isinstance(obj, list) and all(
            [isinstance(it, BatchStructTemplate) for it in obj]))
    ...
    def fillSingleOrMultiple_WithItemData(batchStructTemplates, itemsToAdd):
        BatchStructTemplate.assertIsBatchStructTemplateOrListOfBatchStructTemplates(
            batchStructTemplates)
        if isinstance(batchStructTemplates, list):
            assert len(batchStructTemplates) == len(
                itemsToAdd), 'batchStructTemplates and itemsToAdd dont have the same length'
    ...
    def getBatchStructDictionaryValues(self, dictionary, toTensor=False):
        ...
        for key, value in dictionary.items():
            if isinstance(value, dict):
                returnDict[key] = self.getBatchStructDictionaryValues(value,
                                                                      toTensor=toTensor)
            else:
                if toTensor:
                    toTensorFunc = getattr(self, value.toTensorFunc)
                    returnDict[key] = self.squeezeshape0of1(
                        toTensorFunc(value.values))
                    # goodToHave2 maybe could have found better but more complex solution than squeezeshape0of1; this one has problem with batchSize 1!!!
                else:
                    returnDict[key] = value.values
        return returnDict

    def getBatchStructValues(self, toTensor=False):
        if isinstance(self.dictStruct,
                      BatchStructTemplate_Non_BatchStructTemplate_Objects):  # this is for the case we have made BatchStructTemplate of non dictionary object
            if toTensor:
                toTensorFunc = getattr(self, self.dictStruct.toTensorFunc)
                return self.squeezeshape0of1(
                    toTensorFunc(self.dictStruct.values))
            return self.dictStruct.values
        return self.getBatchStructDictionaryValues(self.dictStruct,
                                                   toTensor=toTensor)

    def getBatchStructTensors(self):
        return self.getBatchStructValues(toTensor=True)

    def getSingleOrMultiple_BatchStructValues(batchStructTemplates,
                                              toTensor=False):
        BatchStructTemplate.assertIsBatchStructTemplateOrListOfBatchStructTemplates(
            batchStructTemplates)
        if isinstance(batchStructTemplates, list):
    ...
    def getSingleOrMultiple_BatchStructTensors(batchStructTemplates):
        return BatchStructTemplate.getSingleOrMultiple_BatchStructValues(
            batchStructTemplates, toTensor=True)
...
class SamplerFor_vAnnTsDataset(Sampler):
    "# cccDevStruct this is created, because neither default dataloader or vAnnDataloader didnt respect indexes of the vAnnDataset"
...
class VAnnTsDataloader(DataLoader):
    ...
    # mustHave2 num_workers>0 problem
    ...
    def __init__(self, dataset, batch_size=64, collate_fn=None, sampler=None,
                 doBatchStructureCheckOnAllData=False, *args, **kwargs):
        ...
        # goodToHave2 I cant put the shuffle=True, because dataloader doesnt accept the shuffle True and the sampler together
    ...
    def doBatchStructureCheckOnAllData(self):
        # qqq do I need this
        pass
        # goodToHave2 this useful to check some custom collate_fns
        # goodToHave2 implement it later
        # goodToHave2 find dictStruct which is sure works on all items of 1epoch, and in case of type incompatibility change the explicit
        # ...type for i.e. "<class 'list'>" to 'MIX'
        # goodToHave3 do it in parallel

    def bestNumWorkerFinder(self):
        pass
        # mustHave2 implement it later
    ...
    def commonCollate_fn(self, batch):
        ...
        BatchStructTemplate.fillSingleOrMultiple_WithItemData(batchStructCopy,
                                                              defaultCollateRes)
        return BatchStructTemplate.getSingleOrMultiple_BatchStructTensors(
            batchStructCopy)
