...
def getElectricityProcessed(backcastLen=192, forecastLen=1):
    df =getDatasetFiles('electricity.csv')#kkk that this file exists when pulled
    ...
    normalizer=NormalizerStack(SingleColsStdNormalizer(['powerUsage','daysFromStart', 'hoursFromStart', 'dayOfMonth']),
                    SingleColsLblEncoder(mainGroups))
...
class ElectricityDeepArDataset(VAnnTsDataset):
    def __getitem__(self, idx):
        inputs={}
        inputs['consumerId']=self.getBackForeCastData(self.data, idx, mode=self.modes.singlePoint, colsOrIndexes=self.consumerId)
        inputs['allReals']=self.getBackForeCastData(self.data, idx, mode=self.modes.backcast, colsOrIndexes=self.allReals)
        inputs['target']=self.getBackForeCastData(self.data, idx, mode=self.modes.backcast, colsOrIndexes=self.target, shiftForward=0)

        outputs=self.getBackForeCastData(self.data, idx, mode=self.modes.backcast, colsOrIndexes=self.target, shiftForward=1)
